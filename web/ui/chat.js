...
export class ChatView {
  ...
  constructor(options = {}) {
    ...
    /** @type {Map<string, { bubble: HTMLElement|null, hist: HTMLElement }>} */
    this._asstByResp = new Map();
    // 字幕气泡跟随数字人真实出声：delta 先缓存在这，检测到出声才显示，
    // 检测到静音才消失（不再用固定 4 秒）。见 onAvatarSound()。
    /** @type {{ text: string, rid: string } | null} */
    this._pendingSubtitle = null;
    /** @type {HTMLElement | null} */
    this._liveAssistantBubble = null;
    /** @type {ReturnType<typeof setTimeout> | 0} 声音检测不可用时的兜底显示定时器 */
    this._subtitleFallbackTimer = 0;
  ...
  _reapBubbles() {
    ...
    for (const el of visible) {
      if (el.classList.contains("assistant")) continue; // 字幕气泡由声音驱动（onAvatarSound），不参与计时 reaper
  ...
  onTranscript(d) {
    ...
    if (d.role === "user") {
      ...
      // 历史记录立即写入（持久对话面板不随音频时序延迟）；
      // 字幕气泡延迟到数字人真实出声再显示（见 onAvatarSound）。
      const entry = this._asstByResp.get(rid);
      if (!entry) {
        this._asstByResp.set(rid, { bubble: null, hist: this._appendHistMsg("assistant", d.text, false) });
      } else {
        this._updateHistMsg(entry.hist, d.text, false);
      }
      this._pendingSubtitle = { text: d.text, rid };
      this._scheduleSubtitleFallback();
      this._markUnread();
    }
  }
  ...
  /**
   * 数字人真实出声状态变化（digital-avatar.js 旁路检测，经 main.js 转发）。
   * 字幕气泡不再用固定 4 秒消失，而是与声音/口型同轨：
   *   speaking=true  → 有声开始：把缓存的待显示字幕放上屏幕（若声音已起）
   *   speaking=false → 回到静音：字幕跟随消失（声音/口型此刻也停了）
   * @param {boolean} speaking
   */
  onAvatarSound(speaking) {
    if (speaking) {
      this._clearSubtitleFallback();
      const p = this._pendingSubtitle;
      if (!p) return; // 没有待显示字幕（理论上字幕先于声音到，这里总会有）
      this._pendingSubtitle = null;
      this._showAssistantSubtitle(p.text, p.rid, false);
    } else {
      this._clearSubtitleFallback();
      if (this._liveAssistantBubble?.isConnected) {
        this._dismissBubble(this._liveAssistantBubble);
      }
      this._liveAssistantBubble = null;
    }
  }

  /** 把字幕气泡放上屏幕。fallback=true 表示声音检测不可用、走固定时长收场。
   *  @param {string} text @param {string} rid @param {boolean} fallback */
  _showAssistantSubtitle(text, rid, fallback) {
    let entry = this._asstByResp.get(rid);
    if (!entry) {
      entry = { bubble: null, hist: this._appendHistMsg("assistant", text, false) };
      this._asstByResp.set(rid, entry);
    }
    if (!entry.bubble || entry.bubble.classList.contains("out")) {
      entry.bubble = this._spawnBubble("assistant", text);
    } else {
      this._updateBubbleText(entry.bubble, text);
    }
    this._liveAssistantBubble = entry.bubble;
    if (fallback) {
      setTimeout(() => {
        if (this._liveAssistantBubble === entry.bubble) {
  ...
  /** 兜底：字幕缓存后长时间等不到「出声」（音频检测不可用等），
   *  超时强制显示，避免字幕永久不出现。 */
  _scheduleSubtitleFallback() {
    if (this._subtitleFallbackTimer) clearTimeout(this._subtitleFallbackTimer);
    this._subtitleFallbackTimer = setTimeout(() => {
      this._subtitleFallbackTimer = 0;
      const p = this._pendingSubtitle;
      if (!p) return;
      this._pendingSubtitle = null;
      this._showAssistantSubtitle(p.text, p.rid, true);
    }, 2500);
  }

  _clearSubtitleFallback() {
    if (this._subtitleFallbackTimer) {
      clearTimeout(this._subtitleFallbackTimer);
      this._subtitleFallbackTimer = 0;
    }
  }
  ...
  onResponseFinished(detail) {
    ...
    if (status === "cancelled") {
      // 被中断的回复不会出声：清掉它缓存的待显示字幕，避免下段声音起来时
      // 误显示一段已被砍掉的文本。
      if (this._pendingSubtitle?.rid === responseId) this._pendingSubtitle = null;
    ...
    // 例外：整段式下 response.done 在音频推送时即到，早于数字人真实出声，此时
    // 还有一条字幕（_pendingSubtitle）等着显示。若在这里删掉 entry，稍后声音探针
    // 触发 _showAssistantSubtitle 时会因找不到条目而再 append 一条相同的历史记录
    // → 每条回复在对话记录里重复显示。保留 entry，让字幕复用已建的历史行即可。
    if (this._pendingSubtitle?.rid !== responseId) {
      this._asstByResp.delete(responseId);
    }
  }
