...
export class S2sWsRealtimeClient extends EventTarget {
  ...
  constructor(options) {
    ...
    /** @type {number} 口型同步音频延迟（ms）：缓冲后再播放，抵消视频管线滞后。
     * 仅用于 WS 直连回退路径；数字人同轨音频（audioFromAvatar）无需补偿。 */
    this._lipSyncDelayMs = options.lipSyncDelayMs ?? 0;
    /** @type {boolean} 声音改由数字人 WebRTC 同轨输出（与口型天然同步）。
     * 开启后 response.output_audio.delta 仍照常接收（用于转写/状态），但不再喂播放。 */
    this._audioFromAvatar = !!options.audioFromAvatar;
    /** @type {WebSocket | null} */
    this._ws = null;
  ...
  async _setupAudio() {
    ...
    playbackNode.port.postMessage({ kind: "config", inputRate: OUTPUT_SAMPLE_RATE, delayMs: this._lipSyncDelayMs });
  ...
  async _onWsMessage(raw) {
    ...
    switch (type) {
      ...
      case "sessionid":
      case "sessionid.request":
        // Orchestrator 反向询问浏览器当前 avatar sessionid（口型同步用）。
        // 浏览器侧 main.js 监听本事件并回传 DigitalAvatar.sessionid。
        this.dispatchEvent(new CustomEvent("sessionid-request"));
        break;
    }
  }
  ...
  /** @param {string} b64 */
  _pushAudioDelta(b64) {
    // 数字人同轨输出声音：这里不再直接播放，避免双声（音频由 #av-audio 播放）。
    if (this._audioFromAvatar) return;
  ...
  /** 实时调整口型同步音频延迟（ms），0=不延迟立即播放 */
  setLipSyncDelay(ms) {
    this._lipSyncDelayMs = ms;
    this._playbackNode?.port.postMessage({ kind: "config", delayMs: ms });
  }
  ...
  /** Send an arbitrary control message to the backend (e.g. relaying the
   *  digital-avatar's LiveTalking sessionid for lip-sync). */
  sendControl(obj) {
    this._send(obj);
  }
