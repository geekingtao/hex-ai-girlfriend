...
class PlayerStreamTrack(MediaStreamTrack):
    ...
    async def recv(self) -> Union[Frame, Packet]:
        ...
        # 空队列不再死等（0.005s 空转会把 WebRTC 输出冻结在 TTS 间隙），改输出"保持帧"：
        # 音频 = 20ms 静音、视频 = 最后一帧，音视频 RTP 时间戳同步前进 → 客户端无冻结、无漂移。
        while True:
            try:
                ...
                if self.kind == 'audio':
                    ...
                    eventpoint = None
                    break
                last = getattr(self._player, '_last_video', None)
                if last is not None:
                    # 保持帧：复用最后一帧继续以 25fps 排出，避免视频轨断流
                    frame = VideoFrame.from_ndarray(last, format="bgr24")
                    eventpoint = None
                    break
                # 尚无首帧（会话刚建立）：短暂等待，不空转烧 CPU
...
class HumanPlayer:
    ...
    def __init__(
        ...
        self._last_video = None  # 空队列保持帧用：缓存最后一帧 ndarray
    ...
    def push_video(self, frame):
        ...
        self._last_video = frame  # 缓存 ndarray 供空队列保持帧复用
