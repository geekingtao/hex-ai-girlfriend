...
class RTCManager:
    ...
    async def _create_pc_and_answer(self, avatar_session, sessionid, offer):
        ...
        任一环节失败都会关闭并回收这个 pc（不能依赖 connectionstatechange——
        连接从未建立时它可能永远不触发），然后重新抛出异常，由调用方回收 session。
        """
    ...
    async def handle_offer(self, request):
        ...
        try:
            ...
            # 头像加载失败（如图片数据缺失）等：拒绝 offer 并回传原因，
            # 避免会话带病建起后推理线程崩掉、画面声音一起消失。
            logger.warning("Rejecting offer (session build failed): %s", e)
            return web.Response(
                content_type="application/json",
                text=json.dumps({"code": -1, "msg": str(e)}),
            )
        ...
        # offer 解析失败（如 aiortc 缺 ICE 字段）会抛异常 → 必须回收刚建的
        # session，否则成为永不清理的孤儿，累积到 max_session 后全部 offer 被拒。
        try:
            pc = await self._create_pc_and_answer(
                session_manager.get_session(sessionid), sessionid, offer
            )
        except Exception as e:
            logger.warning("offer 处理失败，回收 session %s: %s", sessionid, e)
            session_manager.remove_session(sessionid)
            return web.Response(
                content_type="application/json",
                text=json.dumps({"code": -1, "msg": f"offer processing failed: {e}"}),
            )
    ...
    async def handle_whep(self, request):
        ...
        try:
            ...
            logger.warning("whep 处理失败，回收 session %s: %s", sessionid, e)
            session_manager.remove_session(sessionid)
            return web.Response(
                status=503,
                content_type="text/plain",
                text=f"offer processing failed: {e}",
            )
